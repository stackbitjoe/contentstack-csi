...
export default function CarouselSection(props) {
    const { elementId, colors, backgroundImage, badge, titleBlock, subtitle, items = [], arrangement, styles = {} } = props;
    return (
        <Section
            elementId={elementId}
        ...
        >
            <div className={classNames('w-full', 'flex', 'flex-col', mapStyles({ alignItems: styles?.self?.justifyContent ?? 'flex-start' }))}>
                ...
                {titleBlock?.text && (<TitleBlock {...titleBlock} className={classNames('w-full', 'max-w-sectionBody', { 'mt-4': badge?.label })} data-sb-field-path=".titleBlock" />)}
                {subtitle && (
                    <p
