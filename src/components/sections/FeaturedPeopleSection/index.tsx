...
export default function FeaturedPeopleSection(props) {    
    const { elementId, colors, backgroundImage, badge, titleBlock, subtitle, actions = [], people = [], arrangement, styles = {} } = props;
    ...
    return (
        ...
        >
            <div className={classNames('w-full', 'flex', 'flex-col', mapStyles({ alignItems: styles?.self?.justifyContent ?? 'flex-start' }))}>
                ...
                {titleBlock && <TitleBlock {...titleBlock} className={classNames('w-full', 'max-w-sectionBody', { 'mt-4': badge?.label })} data-sb-field-path=".titleBlock" />}
                {subtitle && (
                    <p
                        className={classNames(
                            ...
                            {
                                'mt-4': badge?.label || titleBlock?.text
                            }
                        )}
                ...
                <FeaturedPeopleVariants
                    variant={arrangement}
                    people={people}
                    hasTopMargin={!!(badge?.label || titleBlock?.text || subtitle)}
                    hasSectionTitle={!!titleBlock?.text}
                />
                <FeaturedPeopleSectionActions actions={actions} hasTopMargin={!!(badge?.label || titleBlock?.text || subtitle || people.length > 0)} />
            </div>
        </Section>
    );
}
...
function FeaturedPerson(props) {
    const { elementId, title, image, role, bio, colors = 'bg-light-fg-dark', styles = {}, className, hasSectionTitle } = props;
    return (
        <div
            id={elementId}
            className={classNames(
        ...
        >
            ...
            {title &&
                (hasSectionTitle ? (
                    <h3 data-sb-field-path=".title">{title}</h3>
                ) : (
