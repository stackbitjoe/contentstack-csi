...
export default function GenericSection(props) {
    const { elementId, colors, backgroundImage, badge, titleBlock, subtitle, text, actions = [], media, styles = {} } = props;
    const flexDirection = styles?.self?.flexDirection ?? 'row';
    const alignItems = styles?.self?.alignItems ?? 'flex-start';
    const hasTextContent = !!(badge?.url || titleBlock?.text || subtitle || text || actions.length > 0);
            ...
            >
                {hasTextContent && (
                    ...
                    >
                        ...
                        {titleBlock && <TitleBlock {...titleBlock} className={classNames({ 'mt-4': badge?.label })} data-sb-field-path=".titleBlock" />}
                        {subtitle && (
                            <p
                        ...
                        {text && (
                            <Markdown
                                ...
                                className={classNames('sb-markdown', 'sm:text-lg', styles?.text ? mapStyles(styles?.text) : undefined, {
                                    'mt-6': badge?.label || titleBlock?.text || subtitle
                                })}
                        ...
                        <GenericSectionActions
                            actions={actions}
                            hasTopMargin={!!(badge?.label || titleBlock?.text || subtitle || text)}
