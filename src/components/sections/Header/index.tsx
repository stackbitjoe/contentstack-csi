    ...
    return (
        ...
        >
            <div className="max-w-7xl mx-auto">
                ...
                <HeaderVariants {...props} primaryLinks={props.primaryLinks} secondaryLinks={props.secondaryLinks} />
            </div>
        </header>
    );
}
    ...
    return (
        <>
            {links.map((link, index) => {
                if (link.__metadata.modelName === 'sub_navigation') {
                    return (
                        <li
                            key={index}
                            className={classNames('relative', inMobileMenu ? 'border-t py-3' : 'py-2 group')}
                            onMouseOver={() => !inMobileMenu && setIsSubNavOpen(true)}
                            onMouseLeave={() => !inMobileMenu && setIsSubNavOpen(false)}
                        ...
                        >
                            ...
                            >
                                {link.title}
                                <ChevronDownIcon
                                    className={classNames('fill-current', 'h-4', 'w-4', isSubNavOpen && 'rotate-180', inMobileMenu ? 'ml-auto' : 'ml-1')}
                                />
                            </button>
                            {(link.links ?? []).length > 0 && (
                                <ul
...
function ListOfSubNavLinks({ links = [], inMobileMenu = false }) {
    return (
        <>
            {links.map((link, index) => (
                <li key={index}>
                    <Action {...link} className={classNames(inMobileMenu ? 'w-full justify-start' : 'text-sm')}
                    // data-sb-field-path={`.${index}`} 
                    />
                </li>
            ))}
